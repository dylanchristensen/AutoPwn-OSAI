...
# --- UPDATED: get_ai_next_step with special logic for choose_strategy phase ---
def get_ai_next_step(exploit_state, verbose=True):
    ...
    if phase == 'deploy_shellcode':
        if exploit_state.get('attack_strategy') == 'ret2libc':
            playbook_name = "deploy_shellcode_wx_playbook.txt"
        else:
            playbook_name = "deploy_shellcode_playbook.txt"
    ...
    # --- THE FIX: Special logic to give the AI the info it needs ---
    if phase == 'choose_strategy':
        stack_exec_result = "Not found"
        for entry in reversed(exploit_state["HISTORY"]):
            if entry.get('command') == 'recon_stack_executable':
                stack_exec_result = entry.get('output', 'Error retrieving output')
                break
        user_prompt += f"CRITICAL_INFORMATION:\n- Result of 'recon_stack_executable': {stack_exec_result}\n\n"

    elif phase == 'deploy_shellcode':
        user_prompt += f"EIP_OFFSET: {exploit_state.get('EIP_OFFSET', 0)}\n"
        if exploit_state.get('attack_strategy') == 'esp':
    ...
    user_prompt += "Based on the information provided, provide the JSON for the very next step."
    
    if verbose:
        print(f"[AI] Querying model for next action...\n---\n{user_prompt}\n---")
...
def main():
    ...
    exploit_state = {
        ...
        "crash_size": 0, "EIP_OFFSET": None, "attack_strategy": None,
        ...
        while exploit_state['phase'] != 'done':
            ...
            if current_phase == 'find_offset':
                ...
                if final_offset is not None:
                    ...
                    exploit_state['phase'] = 'choose_strategy'
                else:
                    print("[ORCHESTRATOR] Halting due to failure in offset finding.")
                    exploit_state['phase'] = 'done'
                continue
            
            ai_step = get_ai_next_step(exploit_state, args.verbose)
            ...
            elif action == "set_attack_strategy":
                exploit_state['attack_strategy'] = content
                command_output = f"Attack strategy set to '{content}'."
            ...
            if exploit_state['phase'] == 'recon' and len(exploit_state['recon_completed']) >= 3:
                ...
                try:
                    crash_size_content = exploit_state['HISTORY'][-3]['content']
                    exploit_state['crash_size'] = int(crash_size_content) if str(crash_size_content).isdigit() else 0
                except (IndexError, ValueError):
                    exploit_state['crash_size'] = 0 
                if args.verbose: print(f"[STATE] Initial crash found at size ~{exploit_state['crash_size']}. Moving to FIND_OFFSET phase.")
            elif exploit_state['phase'] == 'find_offset' and exploit_state.get('EIP_OFFSET'):
                exploit_state['phase'] = 'choose_strategy'
                if args.verbose: print("[STATE] Offset found. Moving to CHOOSE_STRATEGY phase.")
            elif exploit_state['phase'] == 'choose_strategy' and exploit_state.get('attack_strategy'):
                exploit_state['phase'] = 'deploy_shellcode'
                if args.verbose: print(f"[STATE] Strategy '{exploit_state['attack_strategy']}' chosen. Moving to DEPLOY_SHELLCODE phase.")
